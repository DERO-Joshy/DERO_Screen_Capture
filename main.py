...
import DEROScreenCaptureTool


class Menu(QMainWindow):
    COLORS = ['Red', 'Black', 'Blue', 'Green', 'Yellow']
    SIZES = [1, 3, 5, 7, 9, 11]
    default_title = "DERO Screen Capture"
    ...
    def __init__(self, numpy_image=None, snip_number=None, start_position=(300, 300, 350, 250)):
        ...
        self.setWindowIcon(QIcon('img/DERO.ico'))
        ...
        self.DEROScreenCaptureTool = DEROScreenCaptureTool.SnippingWidget()
        ...
        if numpy_image is not None and snip_number is not None:
            ...
            self.image = QPixmap("img/background.PNG")
    ...
    # DEROScreenCaptureTool.start() will open a new window, so if this is the first snip, close the first window.
    def new_image_window(self):
        if self.DEROScreenCaptureTool.background:
            self.close()
        self.total_snips += 1
        self.DEROScreenCaptureTool.start()
